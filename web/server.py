...
def create_test_messages():
    db_session = db.getSession(engine)
    message = entities.Message(content="Hola", sent_on="12:20", user_from_id=2, user_to_id=4, user_from="Oteranga", user_to="Max20")
...
def create_message():
    ...
    messages = entities.Message(
        content=c['content'],
        sent_on=c['sent on'],
        user_from_id=c['user from id'],
        user_to_id=c['user to id'],
        user_from=c['user from'],
        user_to=c['user to']
    )
    session = db.getSession(engine)
